...
in mkIf (hasAttr "wiregrill" config.krebs.build.host.nets) {
  ...
  in {
    ...
    privateKeyFile = config.sops.secrets."${config.clan.core.settings.machine.name}-wiregrill.key".path;
    ...
    peers = mapAttrsToList
      ...
      (filterAttrs (_: h:
        ...
        && h.nets.wiregrill.wireguard != null
        && (let r = builtins.tryEval h.nets.wiregrill.wireguard.pubkey; in r.success && r.value != null)
      ) config.krebs.hosts);
  };
}
